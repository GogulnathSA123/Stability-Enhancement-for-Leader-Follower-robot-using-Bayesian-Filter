...
class LineSensors_c {
  ...
  public:
    ...
    void calcCalibratedADC() {
      ...
      for( int sensor = 0; sensor < NUM_SENSORS; sensor++ ) {
              calibrated[sensor]  = constrain((readings[sensor] - minimum[sensor])/ scaling[sensor],0.0,1.0);
              Serial.println(calibrated[0]);
      }
      
    } // End of calcCalibratedADC()

    bool BlackDetected(int Sensor_number)
    {
      calcCalibratedADC();
      
      if(calibrated[Sensor_number] == 0)
      {
        return 1;
      }
      else if(calibrated[Sensor_number] == 1 )
      {
        return 0;
      }
    }
